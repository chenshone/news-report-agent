...
- [pending] 对重点入选内容召开 expert_council（专家出报告→互评→共识→裁决）
...
- **expert_council**: 专家委员会（推荐）：先让各专家独立出具报告，再组织交叉评审与共识讨论，最后由主席定稿
...
`expert_council` 会一次性执行完整的四阶段协作流程：
...
#### 何时必须召开（强制触发）
- 可信度/相关性评估出现 **C/D**，或来源单一、疑似软文/PR
- 关键事实口径不一致、存在冲突说法，或涉及重大数值（融资/性能/用户规模/安全事件）
- 影响面广（行业格局/政策监管/安全风险/资本市场），或用户明确要求深度/争议分析

#### 何时可以不召开（省成本）
- 内容非常简单，且来自 **A 级一手来源**，且用户只要“概览/速览”
- 已有足够多 **A/B 级交叉来源** 支撑，且结论无明显争议  
  （此时可只调用 `summarizer` + `impact_assessor`，必要时再用 `expert_supervisor` 快速整合）
...
调用示例：
```python
task("expert_council", '''请召开专家委员会，对下面这条新闻做最终裁决，并输出可直接用于报告的 Markdown。
...
4. **冲突处理**: 有分歧时优先 `expert_council`；若你已拿到各专家输出，则用 `expert_supervisor` 裁决整合
...
7. **深度分析** → 对重点新闻默认调用 `expert_council`（专家出报告→互评→共识→裁决）；非重点按需派生单专家
8. **反思检查点 3** → 检查分析一致性
9. **结果整合** → 汇总所有分析，生成报告
